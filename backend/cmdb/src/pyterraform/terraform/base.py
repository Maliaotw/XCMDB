    ...
    def get_vars(self, ip):
        ...
        for network_obj in models.NetWork.objects.all():
            if ipaddress.IPv4Address(ip) in ipaddress.ip_network(network_obj.iprange, strict=False):
                return {"network": network_obj.network, "gateway": network_obj.gateway}
...
class Terraform(Base, Network):
    def __init__(self, **kwargs):
        ...
        logging.info(f"{self.hostname}\t{self.__class__.__name__}\t{kwargs}")
    ...
    def complete(self, ret):
        if "complete" in ret:
            logging.info(f"{self.hostname} complete ok")
        else:
            logging.info(f"{self.hostname} complete error")
            logging.error(ret)
            raise Exception(f"complete 錯誤 {self.hostname} {ret}")

    def valid(self, ret):
        string = re.findall(r"Resources:.(.*)\.", ret)[0]
        num = re.findall(rf"(\d+).{self.valid_text}", string)[0]
        if int(num) != 0:
            logging.info(f"{self.hostname} valid ok")
            return True
        else:
            logging.error(f"{self.hostname} valid error")
    ...
    def _init(self):
        ...
        if "Terraform has been successfully initialized!" in ret:
            logging.info(f"{self.hostname} terraform init")
        else:
    ...
    def _plan(self):
        ...
        print(ret)
        plan_string = re.findall(r"Plan:\S+(.*)\.", ret)[0]
        num = re.findall(rf"(\d+).to {self.valid_text}", plan_string)[0]

        if int(num) != 0:
            logging.info(f"{self.hostname} plan ok")
        else:
            logging.error(f"{self.hostname} plan error")
