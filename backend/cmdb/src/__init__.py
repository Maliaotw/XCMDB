...
def create_one(models_class, data, filter_field=None):
    """
    ...
    if filter_field is None:
        filter_field = {}
    result = {"code": 0, "message": "", "obj": ""}

    instance_obj = models_class.objects.filter(**filter_field)
    logging.debug(f"{instance_obj} {data}")
    ...
    if instance_obj:
        ...
        if ret:
            ...
            for i in ret:
                ...
                instance_obj.save()
                result["code"] = 2
                result["message"] = f'更新 {instance_obj} (欄位: "{i}" {before_val} -> {ins_data[i]})'
        else:
            result["code"] = 3
            result["message"] = "沒有變化"
        ...
        result["code"] = 1
        result["message"] = f"新增 {instance_obj}"
