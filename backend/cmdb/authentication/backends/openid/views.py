...
class OpenIDLoginView(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        ...
        nonce = Nonce(redirect_uri=redirect_uri, next_path=self.request.GET.get("next"))
        cache.set(str(nonce.state), nonce, 24 * 3600)

        self.request.session["openid_state"] = str(nonce.state)
        authorization_url = client.get_authorization_url(
            redirect_uri=nonce.redirect_uri, scope="code", state=str(nonce.state)
        )
...
class OpenIDLoginCompleteView(RedirectView):
    def get(self, request, *args, **kwargs):
        ...
        user = authenticate(request=self.request, code=self.request.GET["code"], redirect_uri=nonce.redirect_uri)
