...
try:
    ...
    KeycloakRealm = object
    KeycloakOpenID = object
...
class OpenIDTokenProfile:
    ...
    def __str__(self):
        return f"{self.user.username}'s OpenID token profile"
...
class Client:
    ...
    def realm(self):
        if self._realm is None:
            self._realm = KeycloakRealm(server_url=self.server_url, realm_name=self.realm_name, headers={})
        return self._realm
    ...
    def openid_client(self):
        ...
        if self._openid_client is None:
            self._openid_client = KeycloakOpenID(
                server_url=f"{self.server_url}auth/",
    ...
    def get_authorization_url(self, redirect_uri, scope, state):
        url = self.openid_connect_client.authorization_url(redirect_uri=redirect_uri, scope=scope, state=state)
        return url
    ...
    def authorization_code(self, code, redirect_uri):
        token_response = self.openid_connect_client.authorization_code(code=code, redirect_uri=redirect_uri)
        return token_response

    @ssl_verification
    def authorization_password(self, username, password):
        token_response = self.openid_client.token(username=username, password=password)
        return token_response
