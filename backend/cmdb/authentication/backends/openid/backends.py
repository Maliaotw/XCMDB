...
class OpenIDAuthorizationCodeBackend(BaseOpenIDAuthorizationBackend):
    def authenticate(self, request, **kwargs):
        ...
        if not code or not redirect_uri:
            ...
            return None
        try:
            oidt_profile = client.update_or_create_from_code(code=code, redirect_uri=redirect_uri)
        except Exception as e:
            logger.info(f"Authenticate failed: get oidt_profile: {e}")
            return None
        else:
            # Check openid user single logout or not with access_token
            request.session[OIDT_ACCESS_TOKEN] = oidt_profile.access_token
            user = oidt_profile.user
            logger.info(f"Authenticate success: user -> {user}")
            return user if self.user_can_authenticate(user) else None
...
class OpenIDAuthorizationPasswordBackend(BaseOpenIDAuthorizationBackend):
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        if not username:
            ...
            return None
        try:
            oidt_profile = client.update_or_create_from_password(username=username, password=password)
        except Exception as e:
