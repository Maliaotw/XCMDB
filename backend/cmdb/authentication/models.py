...
class UserLoginLog(models.Model):
    ...
    REASON_CHOICE = (
        (REASON_NOTHING, _("-")),
        (REASON_PASSWORD, _("用戶名/密碼 校驗失敗")),
        (REASON_MFA, _("MFA authentication failed")),
        (REASON_NOT_EXIST, _("用戶名不存在")),
        (REASON_PASSWORD_EXPIRED, _("密碼已過期")),
    )

    STATUS_CHOICE = ((True, _("成功")), (False, _("失敗")))
    ...
    def get_login_logs(cls, date_form=None, date_to=None, user=None, keyword=None):
        ...
        if date_form and date_to:
            login_logs = login_logs.filter(datetime__gt=date_form, datetime__lt=date_to)
        if user:
            login_logs = login_logs.filter(username=user)
        if keyword:
            login_logs = login_logs.filter(
                Q(ip__contains=keyword) | Q(city__contains=keyword) | Q(username__contains=keyword)
            )
        return login_logs
...
class AccessKey(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name="User", on_delete=models.CASCADE, related_name="access_keys"
    )
