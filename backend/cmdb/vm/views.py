...
from authentication.backends.api import IsAdminOrReadOnly, WithBootstrapToken
...
class ClusterModelViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        response.data["template"] = serializers.InstanceListSerializer(
            models.Instance.objects.filter(hw_is_template=True).all(), many=True
        ).data
...
class NetWorkStaticModelViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        response.data["template"] = serializers.InstanceListSerializer(
            models.Instance.objects.filter(hw_is_template=True), many=True
        ).data
...
class InstanceViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ("hw_name", "ip_address", "mac_address", "instance_uuid")
    ordering_fields = ("create_at", "hw_name")
    ...
    def get_terraform_data(self, data, template):
        ...
        context = {
            ...
            "vsphere_resource_pool": "{}/Resources".format(models.Cluster.objects.get(id=data["cluster"]).name),
            "host": "{}".format(models.Host.objects.get(id=data["host"]).name),
    ...
    def create(self, request, *args, **kwargs):
        ...
        tasks_obj = tasks.create_vm.delay(terraform_data)
...
class HostViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ("name", "ansible_all_ipv4_addresses")
    ordering_fields = ("name",)
    permission_classes = [IsAdminOrReadOnly]
...
def echo_once(request):
    logging.debug(f"{__name__} {inspect.stack()[0][3]}")
    ...
    if not request.is_websocket():  # 判斷是不是websocket連接
        ...
        while True:
            ...
            for vm in models.VM.objects.filter(is_finish=False):
                ...
                # 成功啟動 前端頁面會自動刷新
                if vm.status == 1:
                    logging.debug(f"創建成功 {vm} {vm.status}")
                    vm.is_finish = True
                    vm.save()
                    data.update({"check": "2/2 check pass", "is_finish": True})
                    for i in range(5):
                        time.sleep(2)
                        logging.debug(f"頁面自動刷新 第{i}次 {data}")
